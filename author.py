...
def main():
    ...
    parser.add_argument(
        "--story_file",
        type=str,
        default="stories/story_2.yaml",
        help="story yaml to run author on",
    )
    parser.add_argument(
        "--mode",
        type=str,
        default="searches_to_entries",
        help="mode must be one of: searches_to_entries, entries_to_words, entries_graph, searches_graph",
    )
    ...
    if args.mode == "searches_to_entries":
        ...
        print(
            "mode must be one of: searches_to_entries, entries_to_words, entries_graph, searches_graph"
        )
        sys.exit(0)
        ...
        for word in set(tokenize_text(entry["text"])):
            ...
            if word in ret:
                if entry["id"] not in ret[word]["all_entry_ids"]:
                    ...
                    ret[word]["match_entry_ids"] = ret[word]["all_entry_ids"][
                        : story["match_count_limit"]
                    ]
                ...
                ret[word] = {
                    ...
                    "all_entry_count": 1,
                    "match_entry_count": 1,
                    "initial_search": "initial_search" in story
                    and story["initial_search"].lower().strip() == word,
                }
    return dict(
        sorted(ret.items(), key=lambda item: item[1]["all_entry_count"], reverse=True)
    )
        ...
        for w in cur_words:
            ...
            for e in search_to_entries:
                ...
                if cur_entry in s_to_e[w]["match_entry_ids"]:
                    if (cur_entry, e) in bidirectional_edges or (
                        e,
                        cur_entry,
                    ) in bidirectional_edges:
                        continue
...
def searches_graph_to_graphviz(g):
    ...
    for term in g:
        ...
        if len(g[term]["all_entry_ids"]) > len(g[term]["match_entry_ids"]):
            label += (
                ", ("
                + ", ".join(g[term]["all_entry_ids"][len(g[term]["match_entry_ids"]) :])
                + ")"
            )
