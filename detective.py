...
def main():
    ...
    parser.add_argument(
        ...
        help="story yaml to run detective on",
    )
    ...
    while True:
        if state["next_prompt"]["prompt"] == "new_search":
            ...
            res = search_results_prompt(
                state["next_prompt"].get("args", {}), state["read_entry_history"]
            )

            # update game, next prompt
            search_results_prompt_args = state["next_prompt"]["args"]
            state["next_prompt"] = {"prompt": res["selection_type"]}
...
def search_results_prompt(args, read_entry_history):
    ...
    prompt = (
        f"\n{len(match_entries)} matches. Read entry:"
        if all_entry_count <= match_count_limit
    ...
    # third to last
    cli_choices.append(new_search_option_text)
    # second to last
    cli_choices.append(search_history_option_text)
    # last
    cli_choices.append(read_progress_option_text)
    cli = Bullet(
        prompt=prompt,
        choices=cli_choices,
        bullet="",
...
def format_entry_selections(entries, read_entry_history):
    ...
    return [
        ("  " if entry["id"] in read_entry_history else "* ")
        + get_short_date_and_text(entry)
